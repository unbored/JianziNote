...
std::vector<qin::PathData> FTFontReader::GetPath(
  ...
  for (int i = 0; i < outline.n_contours; ++i) {
    ...
    ret.push_back(qin::PathData());
    while (true) {
      // 循环取点直至点的index超过contours数组指示的边界
      qin::PathData &data = ret.back();
      // 读一点
      qin::Point2f pt;
      ...
      if (is_first_pt) {
        ...
        // 在线上，根据当前data的key判断：
        if (data.key == qin::PathKey::Close) {
          // 这是新的一组点，记为LineTo
        data.key = qin::PathKey::LineTo;
        }
        // 总之意味着一组结束
        ret.push_back(qin::PathData());
      } else if (tag & 2) {
        // 三次曲线
        data.key = qin::PathKey::CubicTo;
      } else {
        // 二次曲线，再读入一点
        data.key = qin::PathKey::QuadTo;
      }
      cur_pt_idx++;

      if (cur_pt_idx > outline.contours[i] || cur_pt_idx >= outline.n_points) {
        // 一条曲线数据结束
        while (ret.back().key == qin::PathKey::CubicTo &&
                   ret.back().pts.size() < 3 ||
               ret.back().key == qin::PathKey::QuadTo &&
                   ret.back().pts.size() < 2) {
          // 此处意味着最后一条线的终点应当接着起点
          ret.back().pts.push_back(first_pt);
        }
        // 终点应该是close
        if (ret.back().key != qin::PathKey::Close) {
          ret.push_back(qin::PathData());
        }
