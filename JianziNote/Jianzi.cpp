...
Jianzi::Jianzi(const char *u8_ch)
{
    ...
    {
        // 查询指定名称的减字
        SQLite::Statement jianzi_query(*m_db, ("select * from jianzi where name = " + ch).c_str());
        if (jianzi_query.executeStep())
        {
            ...
            m_type = magic_enum::enum_cast<JianziType>(jianzi_query.getColumn("type").getString())
    ...
    try
    {
        ...
        SQLite::Statement capsule_query(*m_db, ("select * from capsule where name = " + ch).c_str());
    ...
    try
    {
        ...
        SQLite::Statement stroke_query(*m_db, ("select * from stroke where name = " + ch).c_str());
        while (stroke_query.executeStep())
        {
            ...
            SQLite::Statement vertex_query(
                *m_db, "select " JE(type) ", " JE(x) ", " JE(y) ", " JE(belong) " from json_each( ? )");
            vertex_query.bind(1, vertice);
            while (vertex_query.executeStep())
            {
                StrokeVertex v;
                v.type = magic_enum::enum_cast<VertexType>(vertex_query.getColumn("type").getString())
                             .value_or(VertexType::None);
                v.pt.x = vertex_query.getColumn("x").getDouble();
                v.pt.y = vertex_query.getColumn("y").getDouble();
                v.belong = magic_enum::enum_cast<VertexBelong>(vertex_query.getColumn("belong").getString())
...
Jianzi::Jianzi(const Jianzi &other)
{
    *this = other;
}
    ...
    while (str.length() > 0)
    {
        ...
        if (sub_pos != str.end())
        {
            ...
            if (sub_pos != str.begin())
            {
                ...
                try
                {
                    SQLite::Statement jianzi_query(*m_db,
                                                   ("select 1 from jianzi where name = '" + sub + "' limit 1").c_str());
                    if (jianzi_query.executeStep())
                    {
                        // 减字存在，直接存入
                    ...
                    else
                    {
                        SQLite::Statement alias_query(
                            *m_db, ("select jianzi from jianzi_alias where alias = '" + sub + "' limit 1").c_str());
...
std::string Jianzi::ParseNatural(const char *u8_str)
{
    ...
    try
    {
        ...
        while (jianzi_query.executeStep())
        {
            ...
            info.type = magic_enum::enum_cast<JianziType>(jianzi_query.getColumn("type").getString())
                            .value_or(JianziType::Other);
        ...
        while (alias_query.executeStep())
        {
            ...
            info.type = magic_enum::enum_cast<JianziType>(alias_query.getColumn("type").getString())
                            .value_or(JianziType::Other);
        ...
        for (auto &s : node.strokes)
        {
            ...
            for (auto &v : s.vertice)
            {
                ...
                case VertexBelong::Medium:
                    // 计算中间点坐标
                    // 中间点坐标实际上跟着包围框变化
                    v.pt.y = (v.pt.y - m_capsule->tl.y) / (m_capsule->br.y - m_capsule->tl.y) * (br.y - tl.y) + tl.y;
                    break;
                default:
                    break;
                }
...
std::vector<JianziStyler::PathData> qin::Jianzi::RenderPath(const JianziStyler &styler) const
{
...
const char *Jianzi::GetName() const
{
    return m_name.c_str();
}

Jianzi::BorderFlags Jianzi::GetBorderFlags() const
{
    return m_border_flags;
}

int Jianzi::GetSegments() const
{
    return m_v_segments;
}
