...
void StylerFromDb::Load(std::string db_file, std::string styler_name)
{
    ...
    {
        // 查询已有的样式名称
        SQLite::Statement styler_query(*db, "select target from styler_list where name = \"" + styler_name + "\"");
    ...
    try
    {
        ...
        {
            VertexType type = magic_enum::enum_cast<VertexType>(vertex_query.getColumn("type").getString())
                                  .value_or(VertexType::None);
            VertexDesc desc;
            desc.pre_rotate = vertex_query.getColumn("pre_rotate").getDouble();
            desc.dir =
                magic_enum::enum_cast<RotateDir>(vertex_query.getColumn("dir").getString()).value_or(RotateDir::Next);
            // 解析PathGroup的json字串
            ...
            SQLite::Statement group_query(*db, "select " JE(key) ", " JE(offsets) " from json_each( ? )");
            for (int i = 0; i < 2; ++i)
            {
                ...
                while (group_query.executeStep())
                {
                    // 新建一个group
                    PathGroup group;
                    group.key = magic_enum::enum_cast<PathKey>(group_query.getColumn("key").getString())
                                    .value_or(PathKey::MoveTo);
                    // 解析PathOffset的json字串
                    std::string offsets = group_query.getColumn("offsets");
                    SQLite::Statement offset_query(*db, "select " JE(x_rel) ", " JE(x_abs) ", " JE(y_rel) ", " JE(
...
std::vector<JianziStyler::PathData> StylerFromDb::RenderPath(const std::vector<Stroke> &strokes) const
{
...
float StylerFromDb::GetStrokeWidth() const
{
    return c_stroke_width;
}

void StylerFromDb::ProcessVertex(const StrokeVertex &v, float weight, Direction dir, std::vector<PathData> &path) const
{
    // 根据方向加载描述
    VertexDesc desc;
    try
